...
def load_settings(session_id: str) -> dict:
    """Return {"persona_key", "custom_prompt", "rag_enabled"} — sensible defaults if nothing saved yet."""
    with _get_conn() as conn:
        row = conn.execute(
            "SELECT persona_key, custom_prompt, rag_enabled FROM session_settings WHERE session_id = ?",
            (session_id,)
        ).fetchone()
    if row is None:
        return {"persona_key": "default", "custom_prompt": None, "rag_enabled": False}
    return {"persona_key": row[0], "custom_prompt": row[1], "rag_enabled": bool(row[2])}


def save_settings(session_id: str, persona_key: str, custom_prompt: str, rag_enabled: bool):
    with _get_conn() as conn:
        conn.execute("""
            INSERT INTO session_settings (session_id, persona_key, custom_prompt, rag_enabled, updated_at)
            VALUES (?, ?, ?, ?, ?)
        ...
        """, (session_id, persona_key, custom_prompt, int(rag_enabled),
