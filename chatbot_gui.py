...
class AriaGUI:
    ...
    def send_message(self):
        ...
        threading.Thread(target=self._get_response, args=(user_text,), daemon=True).start()

    def _get_response(self, user_text):
        try:
            reply = self.aria.chat(user_text)
        ...
        self.root.after(0, self._show_response, reply, error_msg)

    def _show_response(self, reply, error_msg):
        # Remove the "Aria sedang mengetik..." line
        self._remove_last_system_line()

        if error_msg:
            messagebox.showerror("Error", f"Gagal menghubungi Groq API:\n{error_msg}")
        else:
            self._append_message("Aria", "aria", reply)
